...
1. 設定フォルダ(C:/Users/(ユーザー名)/.soyokaze)を削除する
...
ランチャーを管理者権限で実行することにより、管理者権限プロセスがアクティブな状況でもホットキーが効くようになる。  
そのため、ランチャー自体は管理者権限で実行したいけれど、一方で、(セキュリティ上の理由により)コマンド実行により起動するアプリケーションを管理者権限で動かしたくない場合に使う
