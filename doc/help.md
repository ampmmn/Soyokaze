...
- `同一のフィルタコマンドの同時実行を許可する`
  - [フィルタコマンド](#フィルタコマンドの登録編集)を実行したときに表示されるフィルタ画面が複数表示されるのを抑制する
...
フォルダを限定したEverything検索を行うためのコマンド。

検索対象フォルダや、大文字小文字の区別、検索対象をファイルかフォルダに限定した検索などが可能

ファイル名のパターンだけ指定して、その他の条件を指定しないような、通常のEverything検索であれば、Everything本体を使えばいいが、
検索範囲を限定したいときに、検索条件をEverythingに毎回入力するのは手間なので、そのあたりを省力化する目的のもの。
...
- `コマンドの名前`
  - コマンドを実行するためのキーワード。このコマンドを入力した後に検索文字列を入力すると、該当する項目を絞り込んで表示を行う。
- `コマンドの説明`
  - コメント表示欄に記載される文字列。
- `検索対象フォルダ`
  - 検索対象としたいフォルダのパス
...
- Everything連携(公開しているSDKの機能を利用)して、Everything検索の結果を取得し、候補として表示する
- 機能を利用するためには、Everythingが起動している必要がある
