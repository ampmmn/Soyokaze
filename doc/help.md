...
`file://...`で始まるパスをローカルパスやUNC形式のパスに変換し、変換したパスに対して何かをすることができる。


- `Enter`キー押下 → 変換後のパスをクリップボードにコピー
- `Shift-Enter`キー押下 → 開く(ファイルを実行する)
- `Ctrl-Enter`キー押下 → パスを開く(パスが存在するフォルダをファイラーで開く)

fileプロトコルの例: `file:///C:/Windows/WindowsUpdate.log` → `C:\Windows\WindowsUpdate.log`
