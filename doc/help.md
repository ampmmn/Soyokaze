...
画面に表示されているウインドウのタイトルを候補として表示し、それを選択したら該当するウインドウをアクティブにする  
![](../image/activate_window.png)
  - `Excelワークシート名による選択を有効にする`  
入力画面を表示した時点で開いているExcelワークシート名の一覧を候補として表示し、それを選択したら該当するワークシートをアクティブにする
    - 当然ながら、機能を利用するためにはExcelがインストールされている必要がある

- `Webブラウザのブックマーク選択機能`
  - `有効にする`  
チェックすると、EdgeとChromeにそれぞれ登録しているブックマークの一覧を候補として表示し、それを選択したら、該当するブックマークのURLをブラウザで開く  
![](../image/bookmark.png)
...
実効環境のWebブラウザに登録しているブックマークの名前を候補に表示する。  
