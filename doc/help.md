...
- `未分類`
  - `Cygwinパス変換`
    - チェックすると、Cygwin上のパス表記`/cygdrive/c/...`とWindowsローカルパス`C:\...`を変換するコマンドを表示する
...
入力欄にCygwin上のパス表記でファイルパスを入力すると、対応するWindowsローカルパスに変換するコマンドを表示する

![](../image/cygwin-path1.png)

コマンドを実行すると、Windowsローカルパスをクリップボードにコピーする

また、入力欄にWindowsローカルパス表記でファイルパスを入力すると、対応するCygwin上のパス表記に変換するコマンドを表示する
