...
Excelワークブックファイル(`*.xlsx,*.xls`)の任意の範囲のデータを簡易的な辞書データとして利用し、
...
- メモ
  - `ReadDirectoryChangesW`を用いて対象フォルダの更新を検知する
  - ローカルパスは期待通りに検知できるが、ネットワークパスの更新を検知できるかは不明
    - 手元で試した感じ、できる場合とできない場合があり安定してないように見える
    - 個人的に想定する用途はファイルサーバ上の特定フォルダの更新検知なので、ネットワークパスで使えないと意味ない
...
- `データ範囲`
  - 辞書データとして利用したいデータの範囲を指定する
- `取得`
  - クリックすると、現在Excelで表示・選択している範囲の情報を、`Excelファイル`、`シート名`、`データ範囲`に反映する
- `一行目をヘッダ行として扱う`
  - チェックすると、選択範囲の先頭行をヘッダとみなし、辞書データに含めない
- `テスト`
  - クリックすると、`Excelファイル`/`シート名`/`データ範囲`の内容に基づいて辞書データを読み込み、プレビュー欄に表示する

### (試作)フォルダ更新検知コマンドの登録・編集
