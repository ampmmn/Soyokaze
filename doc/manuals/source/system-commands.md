...
最初に覚えると便利なシステムコマンドは次のとおりです。

- `new`: 新しいコマンドを登録する
- `edit`: 登録済みコマンドを管理する
- `setting`: アプリケーション設定を開く
- `exit`: 本アプリを終了する

`shutdown`、`reboot`、`logoff`、`emptyrecyclebin`などは、実行するとWindowsやファイルに影響するコマンドです。誤操作が心配な場合は、キーワードマネージャで無効にするか、`実行前に確認する`を有効にしてください。
...
削除したファイルは、通常の方法では元に戻せません。必要なファイルがごみ箱に残っていないことを確認してから実行してください。
...
アプリケーション設定で外部ファイラーを設定している場合は外部ファイラーで開く。  
...
実行中のアプリケーションによっては、保存していないデータが失われる可能性があります。
...
設定ファイルの保存先については、インストールの[設定ファイルの保存先](/usage/setup)を参照してください。
