...
チェックすると、ブックマーク検索／Web履歴検索機能において、外部ツールを利用できる
  - `Chromeを使う`  
外部ツールとしてChromeを使う  
  - `別のWebブラウザを指定する`  
...
チェックすると、URLを参照するコマンドを実行したときや、入力欄にURLを直接入力したときに、ここで設定したWebブラウザが使われる。
...
チェックすると、Edgeのお気に入りを検索対象とする
- `Webブラウザ(外部ツール)のブックマークを候補として表示する`  
チェックすると、[外部ツール](#externaltool)で指定したWebブラウザのブックマークを検索対象とする
- `URL文字列を候補の絞り込みに使う`  
...
チェックすると、[外部ツール](#externaltool)で指定したWebブラウザの履歴を検索対象とする
- `検索の上限件数`  
...
- `機能を有効にする`  
チェックすると、[Visual Studio Code履歴](/adhoc-command/vscode)機能が有効になる。
- `プレフィックス`  
機能発動のためのキーワードを指定する。  
