# Keyword Manager

## Overview

本アプリに登録したコマンドを追加、編集、複製、削除するための画面です。

登録したコマンドのキーワードを変更したい場合や、不要になったコマンドを無効にしたい場合も、この画面を使用します。

下記のいずれかの操作を行うと、キーワードマネージャーを表示することができる。

- 入力画面で`edit`コマンドを実行する
- 入力画面で空欄のままEnterキーを押下する
- タスクトレイのアイコンを右クリックし、`キーワードマネージャ`を選択する

#### Description

![](../image/window/keyword-manager/keyword-manager.png)

- `リスト`  
登録済のコマンド一覧が表示される
- `フィルター`  
入力した文字列でリストの表示項目を絞り込める  
空白区切りで複数ワードを指定できる(AND検索)
- `新規作成`  
コマンドを新規登録する。コマンドの種別を選択する画面を表示する
- `編集`  
リスト上で選択したコマンドを設定を変更する。編集画面が表示される。
- `複製`  
リスト上で選択したコマンドを複製する  
(選択中のコマンドの設定内容をもとに、別のコマンドを作ることができる)
- `削除`  
リスト上で選択したコマンドを削除する  

新規作成や編集が終わったら、編集画面の`OK`を押して内容を反映します。変更を反映せずに閉じたい場合は`キャンセル`を押します。

コマンドが候補に表示されない場合は、リスト上でそのコマンドが無効になっていないか、フィルター欄に不要な文字が残っていないかを確認してください。

## Copy And Paste

リスト上でコマンドを選択して、`Ctrl-C`でコマンドをクリップボードにコピーし、
`Ctrl-V`で貼り付けることができる。

リモートデスクトップを経由して複数のPC間で本アプリを使用している場合、
クリップボード経由で一方のアプリ側で定義しているコマンドをもう一方のアプリ側に移すことができます。

コピーしたコマンドには、実行ファイルのパスやURLなどの設定も含まれます。貼り付け後は、移行先の環境に同じファイルやフォルダが存在するか確認してください。

## Command Type

作成するコマンドの種類を選択するための画面。  
コマンドの種類を選択して、OKボタンを押下すると、選択したコマンドの新規作成画面を表示する。

各コマンドの説明については [登録型のコマンド](/user-commands) を参照のこと

![](../image/common/command-type.png)

