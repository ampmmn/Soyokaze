...
例えば、メモ帳を`memo`というキーワードで起動する場合は、`コマンドの名前`に`memo`、`実行ファイル・URLなど`に`notepad.exe`を指定します。登録後に入力画面で`memo`と入力してEnterキーを押すと、メモ帳を起動できます。

このページには詳細な設定項目も含まれています。まずはコマンド名と実行ファイル・URLなどだけを設定し、必要になったときにパラメータやホットキーなどを追加してください。
...
例えば、`browser`というコマンドのパラメータに`$*`を指定すると、入力画面で`browser https://example.com`と入力したとき、URLがそのまま実行ファイルに渡されます。

- `パラメータ候補リストを使用する`  
チェックすると、パラメータに`$*`または`$1`〜`$9`を指定した場合に、このコマンドの実行時引数に対する候補リストをポップアップ表示する。詳細は[パラメータ候補リスト](/window/input-window.md#parameter-candidate-popup)を参照。
...
PNG/JPG/GIF/BMP/ICO形式のファイルが利用可能
  - `URLからファビコンを取得する`    
`実行ファイル・URLなど`にURLが入力されているときのみ表示される。  
選択すると、URLからファビコンを取得し、アイコンとして設定する。
  - `アイコンを初期状態に戻す`  
...
- `正規表現を使用する`  
チェックすると、`ウインドウタイトル`と`ウインドウクラス`の文字列を正規表現パターンとして扱う。利用できる正規表現の制限については[正規表現の制限](/others/regular-expression)を参照する。
- `テスト`  
