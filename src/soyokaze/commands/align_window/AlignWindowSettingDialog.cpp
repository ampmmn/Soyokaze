...
SettingDialog::SettingDialog(CWnd* parentWnd) : 
	launcherapp::control::SinglePageDialog(IDD_COMMAND_ALIGNWINDOW, parentWnd),
...
void SettingDialog::SetItemToList(int index, const Param::ITEM& item)
{
  ...
  CString text;
	...
	if (item.mAction == ACTION::AT_SETPOS) {
		...
		if (item.mAction == ACTION::AT_MAXIMIZE) {
			listWndPtr->SetItemText(index, 2, _T("(最大化)"));
		}
		else if (item.mAction == ACTION::AT_MINIMIZE){
			listWndPtr->SetItemText(index, 2, _T("(最小化)"));
		}
		else if (item.mAction == ACTION::AT_HIDE){
