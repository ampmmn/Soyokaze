#include "pch.h"
#include "UIElementIndicatorWindow.h"
...
UIElementIndicatorWindow::UIElementIndicatorWindow()
{
}
...
UIElementIndicatorWindow* UIElementIndicatorWindow::GetInstance()
{
	static UIElementIndicatorWindow inst;
	return &inst;
}

// 指定したhwndの領域を覆う形でウインドウを表示
void UIElementIndicatorWindow::Indicate(HWND hwnd, const CRect& rc)
{
	if (mHwnd == nullptr) {
		mHwnd = Create();
	}
...
void UIElementIndicatorWindow::ClearIndication()
{
	ShowWindow(mHwnd, SW_HIDE);
}

LRESULT CALLBACK UIElementIndicatorWindow::OnWindowProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	if (msg == WM_PAINT) {

		auto thisPtr = (UIElementIndicatorWindow*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
		...
		if (rc.left != thisPtr->mRectWindow.left || rc.top != thisPtr->mRectWindow.top) {
			...
			spdlog::info("[UIElementIndicatorWindow] window position reset {0},{1} -> {2},{3}",
...
HWND UIElementIndicatorWindow::Create()
{
	spdlog::info("UIElementIndicatorWindow create.");
