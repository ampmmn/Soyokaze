...
CString UIAutomationAdhocCommand::GetGuideString()
{
	return _T("⏎:クリック");
}
...
BOOL UIAutomationAdhocCommand::Execute(Parameter* param)
{
	// クリック位置
...
// メニューの表示名を取得する
bool UIAutomationAdhocCommand::GetMenuItemName(int index, LPCWSTR* displayNamePtr)
{
	if (index == 0) {
		static LPCWSTR name = L"選択(&S)";
		*displayNamePtr= name;
		return true;
	}
	return false;
...
// メニュー選択時の処理を実行する
bool UIAutomationAdhocCommand::SelectMenuItem(int index, launcherapp::core::CommandParameter* param)
{
	if (index != 0) {
		return false;
	}
