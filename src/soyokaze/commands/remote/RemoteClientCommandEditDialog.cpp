#include "pch.h"
#include "framework.h"
#include "RemoteClientCommandEditDialog.h"
...
CommandEditDialog::CommandEditDialog(CWnd* parentWnd) : 
	launcherapp::gui::SinglePageDialog(IDD_REMOTECLIENT, parentWnd)
{
	SetHelpPageId(_T("RemoteClientEdit"));
}
...
void CommandEditDialog::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_HOST, mParam.mHost);
	DDX_Text(pDX, IDC_EDIT_LAUNCHERAPPDIR, mParam.mLauncherAppDir);
...
BEGIN_MESSAGE_MAP(CommandEditDialog, launcherapp::gui::SinglePageDialog)
	...
	ON_EN_CHANGE(IDC_EDIT_HOST, OnUpdateStatus)
	ON_EN_CHANGE(IDC_EDIT_LAUNCHERAPPDIR, OnUpdateStatus)
...
bool CommandEditDialog::UpdateStatus()
{
	...
	if (mParam.mHost.IsEmpty()) {
		mMessage = _T("ホストを設定してください");
		GetDlgItem(IDOK)->EnableWindow(FALSE);
		return false;
	}
	if (mParam.mLauncherAppDir.IsEmpty()) {
		mMessage = _T("アプリのパスを設定してください");
		GetDlgItem(IDOK)->EnableWindow(FALSE);
		return false;
	}
