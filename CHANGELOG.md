...
- #286 初回起動時に設定ファイルの保存先を選択できるようにした

### Fixed

- #287 追加候補がなくなった後にEnterキーでコマンドを実行できない問題を修正

## 0.59.0 - 2026-09-25

### Added

- #102 モニター構成ごとにランチャーウインドウの位置を記憶
- #160 Everything検索コマンドの候補にコンテキストメニューを追加
- #279 通常コマンドごとにパラメータ候補リストの表示を切り替える機能を追加
- #283 パラメータ候補リストの表示を設定で切り替える機能を追加
- #285 バージョン更新チェック機能を追加
...
- #276 通常コマンドでexe/bat以外の拡張子を管理者権限で実行できない

## 0.58.0 - 2026-09-23

### Added

- #268 環境変数/従来のネットワーク接続画面などを表示するためのコマンド(システムツールコマンド)を追加
- #269 背景画像設定機能を追加
- #271 MathyPadによる単位付き計算機能を追加
- #272 ms-windows-store://... 系の項目を追加する
- #274 実行時引数に対する追加の候補欄を表示する

### Changed

- #267 C/Migemoのsdict読み込みに対応する
- #273 Commandクラスの変更(コマンドが引数を受け入れるか、保持するパスなどを展開可能か問い合わせるAPIを追加)

- `登録しなくても使える機能`>`単位変換` を廃止
  - ここで実装していた機能を `電卓`>`単位付き計算機能`で代替するため

- リリース版について同梱のC/Migemoバイナリを1.8.0に更新 / yet-another-migemo-dictの辞書データをUniDic 2025-12-31版に更新
  - C/Migemoバイナリと辞書についてはリポジトリに含めていないので、この項目についてのリポジトリ上の変更点は無し

### Fixed

- #275 ファイルのフルパスと後続パラメータをまとめて張り付けたとき、「対象候補なし」になる
  - 直ってなかった(0.58.1の次のバージョンで修正)

## 0.57.0 - 2026-09-13

### Added

- #261 プラグインごとの有効/無効および優先度を設定する機能を追加
- #262 プラグインから設定されたファイラーでフォルダを開くAPIを追加
- #263 `shell:` URIスキームに対応
- #255 通貨単位を変換して表示する機能を追加
- #264 グループコマンドの機能拡張
- #265 コマンドラインオプションに`-`で始まる代替形式を追加

### Changed

- #253 固定パターンおよび置換を伴わない正規表現判定にRE2を使用するよう変更
  - RE2非対応の正規表現構文についてはマニュアルに記載
- #190 Chrome/Edgeの最後に使用したプロファイルのブックマークとWeb履歴に対応
  - `Local State`の読み取りに失敗した場合は`Default`プロファイルを使用する
  - ブラウザ側でプロファイルを切り替えた場合、アプリ設定の再読み込み時に変更後のプロファイルを反映する
- #254 アプリケーション設定マニュアルを設定画面ごとに分割
- #256 実行ファイルのディレクトリにあるプラグインもロード対象に追加
- #257 プラグイン情報の取得とAPIバージョン検証に対応
- #259 プラグインからアイコン取得機能を利用できるAPIを追加

### Fixed

- #266 カーソル位置によるアクティブ切替機能 の動作不具合修正
  - ウインドウサイズ変更中にカーソル位置によるアクティブ切替が発動する
  - 入力欄を空にした際のウインドウ縮小で、意図せず非アクティブになる
...
- #247 プラグイン対応
- #246 URLからファビコンを取得しコマンドのアイコンとして設定する機能
- #249 定型文グループコマンドに任意のアイコンを設定する機能
- #251 アイコン選択でURLから画像ファイルを取得できるようにする
- カーソル位置によるアクティブ切替機能を追加

### Changed

- #248 自前のBase64デコードをやめてsimdutfを使う
- 定型文コマンドを非表示化
  - 作成済のコマンドは使用できるが、新規作成は不可。
  - 今後は定型文グループコマンドで代替
  - `newsnippet`コマンドも廃止

### Fixed

- 新しいVSCodeバージョンの履歴データを扱えなくなっていたため追従した
- AliasCommandのテキスト貼付けを実施したときにたまにキャレットが全選択状態になってしまうことがある
- 定型文コマンドの編集画面をリサイズしたとき、メッセージ欄がリサイズに追従しない
- #252 アイコン画像の透過情報が失われることがある問題を修正

## 0.55.0 - 2026-08-11

### Added

- 【クリップボード履歴】プレフィックスなしで履歴検索できるようにする / 検索を発動する最小文字数の設定を追加
- リソース使用量ログを出力する機能を追加

### Changed

- `ウインドウ切替コマンド`を`ウインドウ操作コマンド`に名称変更
- ウインドウ操作コマンドの処理対象として「現在アクティブなウインドウ」を追加
- ExplorePathCommandに対するキャッシュ機能を追加

### Fixed

- #245 UNCパスにアクセスできなかったときに認証を試みるのをやめる
- #244 アプリを再起動するとOneNoteを起動できなくなる
- URLDirectoryIndexCommandでサーバパスワードとプロキシパスワードを平文のまま保持しない
- 一時的に保持した平文パスワードについてスコープを抜ける際に0で埋める
- メモリリークを修正
...
- #237 ウインドウ切替コマンドに位置・サイズを指定する機能を追加 / ウインドウ選択切替の機能を登録型コマンドのウインドウ切替コマンドに統合

### Changed

- 定型文グループコマンド、Web検索コマンド、エイリアスコマンドの設定画面レイアウトを修正
- Web検索コマンドのデフォルトアイコンを変更  
デフォルトのブラウザアイコン→shell32.dllの虫眼鏡アイコン
- 一部のコマンドの説明文を微修正
- #240 ホットキーのUI周りを修正
...
- #242 アプリ終了時に一時ファイルを消していない
- #241 定型文グループコマンド 設定画面でアイテムを削除すると「名前が重複している定型文があります」というエラーがでる
- ウインドウ選択切替において、不可視なUWPアプリが候補として表示されてしまう
- ウインドウの位置をリセットしたときに、ウインドウ高さがおかしい
...
- [Win32メニュー項目検索] 表示中の全てのウインドウを対象にするオプションを追加

### Fixed

- Python拡張コマンドをホットキー経由で実行したときにスクリプトの内容が期待通りに動作しない場合がある

## 0.52.2 - 2026-02-26

### Fixed

- #232 VSCodeの履歴コマンドが動作しない
- #233 候補リストの行高さが縮んで文字が重なる

## 0.52.1 - 2026-02-18

### Changed

- #227 Win32メニュー項目に対するキャッシュ/別名設定機能を実装

## 0.52.0 - 2026-02-15

### Added

- #225 キーワードマネージャのフィルター欄で文字を入力するたびにリストを更新しない
- #103 #227 画面上のUI要素をラベルで検索して実行(クリック、選択)する機能

### Changed

- #228 他のコマンドを選択するときのUIの見直し
- 定型文グループコマンドの一致判定対象として、説明やテキストを含める
  - 従来は名前のみを使って一致判定をしていた
- 定型文グループコマンドを編集するときの初期フォーカスをリストに設定する

### Fixed

- #231 `launcher_proxy`側でのOneNoteページ一覧取得を非同期で行う
- フィルタコマンドや簡易辞書コマンド経由で別のコマンドを実行したとき、コマンド実行履歴への登録を行わない
- ヘルプウインドウのwebview2領域をクリックしないとキー操作によるスクロールができない
- あふwからの情報取得ができない
...
- #221 Ctrl-C,Ctrl-Vによるコマンドのコピペ機能(リモートデスクトップ経由でのコマンドのエクスポートとインポート)

### Changed

- #218 【仕様変更】アクティブウインドウのモニター中央に入力画面を表示する
- ディレクトリ名の変更`gui`→`control`
- #220 ヘルプウインドウを専用のウインドウで開く

### Fixed

- #217 AfxW_GetCurrentDirで落ちることがある
- #222 プレフィックスを設定しているとOneNoteコマンドでページ名検索できない
- #224 キーワードマネージャでコマンドを削除したあと、ダイアログをキャンセルした場合に、削除したコマンドが元に戻らない

## 0.50.0 - 2026-01-04

### Added

- #215 key.writeのキー入力でマルチバイト文字を入力できるようにする

### Changed

- #216 定型文グループ設定時に一つの画面で定型文を登録できるようにする
...
- #214 ファイルやフォルダを任意のコマンドで開けるようにする
...
- #213 候補がない状態でTabキーを押すとフォーカスが外れてしまう

## 0.48.2 - 2025-12-22

### Fixed

- #211 フォルダ/ファイルを実行するときは履歴登録しない

## 0.48.1 - 2025-12-21

### Added

- #210 通常コマンドがフォルダパスを指す場合、フォルダの子要素を列挙できるようにする

### Fixed

- #209 フォルダ内要素を列挙するときに前方一致の要素が優先されない
...
- sqlite3ファイルを新規作成する系の機能が動作しない(例: クリップボード履歴)
...
- 0.47.0で追加した処理について、特定ケースで落ちる不具合を修正
- #205 ウインドウ選択切替でウインドウを閉じた後、枠が残ることがある
...
- #200 Outlookフォルダ選択機能
- #202 通常コマンドで起動するアプリ(のウインドウ)がすでに存在する場合はウインドウ切替できるようにする
- #198 修飾キーの押下状態に応じてガイド欄の項目を強調表示する
- #204 配色設定でUndoできるようにする

### Changed

- #195 tidy-html5を静的リンクではなくDLLとして利用するようにする
- #199 Python拡張コマンドで実行したスクリプト内で実行時引数を受け取れるようにする

### Fixed

- #196 フィルタコマンドの後段アクションとして振り分けコマンドを設定したとき振り分けが機能しない
- #201 アプリ設定画面を表示したとき前面に表示されない
- #203 コマンド設定画面をリサイズするとコントロールのレイアウトが崩れる
...
- #187 【ウインドウ選択切替】ウインドウを閉じた後に入力ウインドウを閉じないようにする
- #188 URLを直接入力して実行したときに履歴に登録するのをやめる
...
- #180 認証されていないネットワークパスを無効なパスとして扱わないようにする / 無効なマクロ記述があったときに該当箇所を空に置換しない
...
- #146 中ボタンクリックでクリップボードのテキストを張り付けする
